...
saber11_2018 <- read_csv(
  file.path(ruta_limpio, "saber11_2018_limpio.csv"),
...
# **MÓDULO 2 — Limpieza completa y selección de 32 variables**

```{r}
limpiar_saber11_2018 <- function(df) {
  ...
  df %>%  
    ...
    mutate(
      FAMI_PERSONASHOGAR = case_when(
        ...
        FAMI_PERSONASHOGAR == "7 a 8" ~ "7-8",
        FAMI_PERSONASHOGAR == "9 o más" ~ "9+",
        TRUE ~ NA_character_
      ),
...
clean_2018 <- limpiar_saber11_2018(saber11_2018)
glimpse(clean_2018)
```
...
# Preparación y matriz de métodos:
...
# **MÓDULO 4 — Aplicar imputación a las 5 bases**

```{r}
bases <- paste0("imp2018_db", 1:5)

for (i in 1:5) {

  archivo <- file.path(ruta_imp, paste0("imp2018_db", i, ".csv"))
  df <- read_csv(archivo, show_col_types = FALSE)

  df_imp <- imputar_unafase(df)

  write_csv(df_imp,
            file.path(ruta_imp, paste0("imp2018_db", i, "_IMPUTADA.csv")))

  cat("Base imputada guardada: imp2018_db", i, "_IMPUTADA.csv\n")
}
```



MÓDULO 5 — Estandarización final
...
for (i in 1:5) {
  ...
  archivo <- file.path(ruta_imp, paste0("imp2018_db", i, "_IMPUTADA.csv"))
  df <- read_csv(archivo, show_col_types = FALSE)
...
for (i in 1:5) {
  ...
  archivo_csv <- file.path(ruta_imp, paste0("imp2018_db", i, "_IMPUTADA.csv"))
  df <- read_csv(archivo_csv, show_col_types = FALSE)

  saveRDS(df, file.path(ruta_imp, paste0("imp2018_db", i, ".rds")))

  write.xlsx(df, file.path(ruta_imp, paste0("imp2018_db", i, ".xlsx")), overwrite = TRUE)
...
# **MÓDULO 7 — Verificación final**

```{r}
for (i in 1:5) {

  df <- read_csv(file.path(ruta_imp, paste0("imp2018_db", i, "_IMPUTADA.csv")))
