...
ruta_limpio <- file.path(ruta_base, "2_sinDuplicados")
ruta_imp <- file.path(ruta_base, "3_DB_imp")

saber11_2017 <- read_csv(
  file.path(ruta_limpio, "saber11_2017_limpio.csv"),
  show_col_types = FALSE
)

bases <- paste0("imp2017_db", 1:5)
...
# **MÓDULO 2 — Limpieza completa y creación del dataset limpio (32 variables)**

```{r}
limpiar_saber11_2017 <- function(df) {

  df %>%  
    select(all_of(vars_32)) %>%
    ...
    mutate(
      FAMI_PERSONASHOGAR = case_when(
        FAMI_PERSONASHOGAR == "1 a 2" ~ "1-2",
        FAMI_PERSONASHOGAR == "3 a 4" ~ "3-4",
        FAMI_PERSONASHOGAR == "5 a 6" ~ "5-6",
        FAMI_PERSONASHOGAR %in% c("7 a 8","9 o más") ~ "7+",
        TRUE ~ NA_character_
      ),
    ...
    mutate(
      FAMI_CUARTOSHOGAR = case_when(
        FAMI_CUARTOSHOGAR %in% c("Uno") ~ "1",
        FAMI_CUARTOSHOGAR %in% c("Dos") ~ "2",
        FAMI_CUARTOSHOGAR %in% c("Tres") ~ "3",
        TRUE ~ "4+"
      ),
    ...
    mutate(across(
      c(COLE_AREA_UBICACION, COLE_DEPTO_UBICACION, COLE_MCPIO_UBICACION,
...
clean_2017 <- limpiar_saber11_2017(saber11_2017)
glimpse(clean_2017)

```

# **MÓDULO 3 — Imputación MICE (Una sola fase)**
...
imputar_unafase <- function(df) {
  ...
  met[c("COLE_BILINGUE",
        "FAMI_TIENEINTERNET","FAMI_TIENEAUTOMOVIL",
        "FAMI_TIENECOMPUTADOR","FAMI_TIENELAVADORA")] <- "logreg"

  met[c("FAMI_PERSONASHOGAR","FAMI_CUARTOSHOGAR",
        "FAMI_EDUCACIONMADRE","FAMI_EDUCACIONPADRE",
        "FAMI_ESTRATOVIVIENDA")] <- "polr"

  imp <- mice(df, m = 5, maxit = 5, method = met, seed = 5477)
...
# **MÓDULO 4 — Aplicar imputación a las 5 DB**

```{r}
for (i in 1:5) {

  archivo <- file.path(ruta_imp, paste0("imp2017_db", i, ".csv"))
  df <- read_csv(archivo, show_col_types = FALSE)

  df_imp <- imputar_unafase(df)

  write_csv(df_imp, file.path(ruta_imp, paste0("imp2017_db", i, "_IMPUTADA.csv")))

  cat("Base imputada guardada: imp2017_db", i, "_IMPUTADA.csv\n")
}
```


# **MÓDULO 5 — Estandarización final de valores**

```{r}
unificar_valores <- function(df) {

  bin_vars <- c("COLE_BILINGUE","FAMI_TIENEINTERNET",
                "FAMI_TIENEAUTOMOVIL","FAMI_TIENECOMPUTADOR",
                "FAMI_TIENELAVADORA")
...
for (i in 1:5) {
  ...
  archivo <- file.path(ruta_imp, paste0("imp2017_db", i, "_IMPUTADA.csv"))
  df <- read_csv(archivo, show_col_types = FALSE)
...
# MÓDULO 6 — Exportación CSV, XLSX y RDS

```{r}
for (i in 1:5) {

  archivo_csv <- file.path(ruta_imp, paste0("imp2017_db", i, "_IMPUTADA.csv"))
  df <- read_csv(archivo_csv, show_col_types = FALSE)

  saveRDS(df, file.path(ruta_imp, paste0("imp2017_db", i, ".rds")))

  write.xlsx(df, file.path(ruta_imp, paste0("imp2017_db", i, ".xlsx")), overwrite = TRUE)

  cat("Exportada DB", i, "en CSV, XLSX y RDS\n")
}

```


# **MÓDULO 7 — Verificación final (32 variables + nulos)**

```{r}
for (i in 1:5) {

  df <- read_csv(file.path(ruta_imp, paste0("imp2017_db", i, "_IMPUTADA.csv")))

  cat("\nDB", i, "dimensiones:", dim(df), "\n")
